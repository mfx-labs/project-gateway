...
test('control-plane static guard: no AuthoritativeAuditEvent publication and no WP-8 writer-lock API', () => {
  for (const file of controlPlaneFiles) {
    ...
    // outside SLICE_1_PUBLISH_CLASSES and no audit module is imported).
...
test('control-plane static guard: no Slice-2+ production vocabulary', () => {
  for (const file of controlPlaneFiles) {
    const content = readFileSync(file, 'utf8');
    const path = rel(file);
    // Read-only currentness consumption of revocation/supersession state is
    // contract-sanctioned in the decision core only (contract §10/§12).
    if (path === 'src/control-plane/core.ts') {
      ...
      for (const forbidden of ['revocation-record', 'supersession-record', "'revoke'", 'revoke(']) {
    ...
    for (const forbidden of [
      ...
      'ExecutionSummaryRecord', 'verifyCurrentLifecycleState', 'orchestration', 'recordExecutionAttempt',
      'pgw:o:', 'reserved_occurrence', 'decideActivation', 'issueRuntimeGrant', 'pi-guard', 'pi_guard',
    ]) {
      assert.equal(content.includes(forbidden), false, `${path} must not contain Slice-2+ vocabulary (${forbidden})`);
    }
  }
});

test('control-plane static guard: the only publishable record classes are the three Slice-1 classes', () => {
  ...
  assert.equal(boundary.includes('SLICE_1_PUBLISH_CLASSES'), true);
});
